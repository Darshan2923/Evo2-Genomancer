...
def run_brca1_analysis():
    ...
    return {'variants':brca1_subset.to_dict(orient='records'),'auroc':auroc,'plot':img_str}
...
def brca1_example():
    ...
    if "plot" in response:
        ...
        with open("brca1_analyisis_plot.png","wb") as f:
            f.write(plot_data)
...
def get_genome_sequence(position,genome:str,chromosome:str,window_size:int=8192):
    import requests
    ...
    if len(sequence)!=expected_length:
        print(f"Unexpected sequence length: {len(sequence)} (expected: {expected_length})")
...
class Evo2Model:
    ...
    def analyze_single_variant(self, variant_position:int,alternative:str,genome:str,chromosome:str):
        ...
        window_sequence, seq_start = get_genome_sequence(position=variant_position,genome=genome, chromosome=chromosome, window_size=WINDOW_SIZE)

        print(f"Fetched genome sequence window, first 100:{window_sequence[:100]}")

        relative_pos=variant_position - seq_start-1
        print(f"Relative position of variant in window: {relative_pos}")

        if relative_pos<0 or relative_pos>=len(window_sequence):
            raise ValueError(f"Variant position is out of bounds of the fetched genome window. (start={seq_start+1}, end={seq_start+len(window_sequence)})")
        
        reference=window_sequence[relative_pos]
